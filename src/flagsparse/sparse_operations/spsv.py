...
def _spsv_csr_level_kernel(
    ...
    row_nnz = end - start
    n_seg = (row_nnz + BLOCK_NNZ - 1) // BLOCK_NNZ
...
@triton.jit
def _spsv_csr_level_kernel_legacy(
    data_ptr,
    indices_ptr,
    indptr_ptr,
    b_ptr,
    ...
    """Original SpSV row kernel (constexpr MAX_SEGMENTS); avoids tl.range on some Triton builds."""
    pid = tl.program_id(0)
    if pid >= n_level_rows:
        return
    row = tl.load(rows_ptr + pid)
...
def _build_spsv_levels(indptr, indices, n_rows, lower=True):
    ...
    Vectorized inner loop (torch on CPU) avoids per-entry ``.item()`` and scales better
    on million-row matrices.
    """
    if n_rows == 0:
        return []
    indptr_h = indptr.to(torch.int64).cpu().contiguous()
    indices_h = indices.to(torch.int64).cpu().contiguous()
    levels = torch.zeros(n_rows, dtype=torch.int64)
    if lower:
    ...
    for r in range(n_rows):
        buckets[levels_list[r]].append(r)
...
def _triton_spsv_csr_vector(
    ...
    rs = max((max_nnz_per_row + block_nnz_use - 1) // block_nnz_use, 1)
    while rs > 2048 and block_nnz_use < 65536:
        block_nnz_use *= 2
        rs = max((max_nnz_per_row + block_nnz_use - 1) // block_nnz_use, 1)
...
def _triton_spsv_csr_vector_legacy(
    data,
    indices,
    indptr,
    b_vec,
    ...
    row_lengths = indptr[1:] - indptr[:-1]
    max_nnz_per_row = int(row_lengths.max().item()) if n_rows > 0 else 0
    block_nnz_use = int(block_nnz) if block_nnz and int(block_nnz) > 0 else 256
    ...
    for rows_lv in levels:
        ...
        if n_lv == 0:
            continue
        grid = (n_lv,)
        _spsv_csr_level_kernel_legacy[grid](
            data,
            indices,
            indptr,
            b_vec,
...
def flagsparse_spsv_csr(
    ...
    use_legacy=True: original constexpr MAX_SEGMENTS kernel (bak-flagsparse-bak style).
    """
    ...
    if use_legacy:
        if b_in.ndim == 1:
            x = _triton_spsv_csr_vector_legacy(
                data_in,
                kernel_indices,
                kernel_indptr,
                b_in,
            ...
            for j in range(b_in.shape[1]):
                cols.append(
                    _triton_spsv_csr_vector_legacy(
                        data_in,
                        kernel_indices,
                        kernel_indptr,
                        b_in[:, j],
...
def flagsparse_spsv_coo(
    ...
    **kwargs,
    ...
    use_legacy=True forces legacy constexpr CSR kernel after COO->CSR (same as bak).
    """
...
def flagsparse_spsv_csr_bak(*args, **kwargs):
    """SpSV CSR using legacy constexpr-row kernel (bak-flagsparse-bak style). Same as ``use_legacy=True``."""
    kw = dict(kwargs)
    kw["use_legacy"] = True
    return flagsparse_spsv_csr(*args, **kw)


def flagsparse_spsv_coo_bak(*args, **kwargs):
    """SpSV COO via sorted COO→CSR then legacy CSR kernel. Same as ``use_legacy=True``."""
    kw = dict(kwargs)
    kw["use_legacy"] = True
    return flagsparse_spsv_coo(*args, **kw)
